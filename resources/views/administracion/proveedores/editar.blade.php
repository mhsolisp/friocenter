...
<head>
    ...
    <title>Editar proveedor — Frío Center</title>
    <style>
...
<main>
    ...
    <div class="card">
        <form action="{{ route('administracion.proveedores.update', $proveedor) }}" method="POST">
            @csrf

            <label for="razon_social">Razón social</label>
            <input type="text" name="razon_social" id="razon_social" required value="{{ old('razon_social', $proveedor->razon_social) }}">

            <div class="fila-2">
                <div>
                    <label for="cuit">CUIT</label>
                    <input type="text" name="cuit" id="cuit" required value="{{ old('cuit', $proveedor->cuit) }}">
                </div>
                <div>
                    ...
                    <select name="rubro_id" id="rubro_id">
                        ...
                        @foreach ($rubros as $rubro)
                            <option value="{{ $rubro->id }}" {{ (string) old('rubro_id', $proveedor->rubro_id) === (string) $rubro->id ? 'selected' : '' }}>{{ $rubro->nombre }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="radio-group">
                <label class="radio-option">
                    <input type="radio" name="condicion_fiscal" value="consumidor_final" {{ old('condicion_fiscal', $proveedor->condicion_fiscal) === 'consumidor_final' ? 'checked' : '' }}> Consumidor final
                </label>
                <label class="radio-option">
                    <input type="radio" name="condicion_fiscal" value="factura" {{ old('condicion_fiscal', $proveedor->condicion_fiscal) === 'factura' ? 'checked' : '' }}> Factura
                </label>
            </div>
            ...
            <textarea name="observaciones" id="observaciones">{{ old('observaciones', $proveedor->observaciones) }}</textarea>
