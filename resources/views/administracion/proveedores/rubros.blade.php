...
<head>
    ...
    <title>Rubros de proveedor — Frío Center</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .rubro-fila { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0; border-top: 1px solid var(--line); }
        .rubro-fila:first-of-type { border-top: none; }
        .rubro-nombre { font-weight: 700; color: var(--navy); }
        .rubro-count { font-size: 0.78rem; color: #5b6b78; margin-left: 0.4rem; font-weight: 400; }
        ...
        .sin-rubros { color: #5b6b78; font-size: 0.88rem; padding: 0.5rem 0; }
    </style>
</head>
<body>
<header>
...
<main>
    <div class="top-row">
        ...
        <a href="{{ route('administracion.proveedores.index') }}" class="volver">← Volver a proveedores</a>
    </div>
    ...
    <div class="card">
        <form action="{{ route('administracion.proveedores.rubros.store') }}" method="POST" class="fila-form" style="margin-bottom: 1rem;">
            @csrf
            <input type="text" name="nombre" placeholder="Nuevo rubro, ej. Repuestos" required>
            <button type="submit" class="agregar">Agregar rubro</button>
        </form>

        @if ($rubros->isEmpty())
            <p class="sin-rubros">Todavía no hay rubros cargados.</p>
        @else
            @foreach ($rubros as $rubro)
                <div class="rubro-fila">
                    <span class="rubro-nombre">{{ $rubro->nombre }}<span class="rubro-count">{{ $rubro->proveedores_count }} proveedor{{ $rubro->proveedores_count === 1 ? '' : 'es' }}</span></span>
                    <form action="{{ route('administracion.proveedores.rubros.destroy', $rubro) }}" method="POST"
                          onsubmit="return confirm('¿Eliminar el rubro {{ $rubro->nombre }}? Los proveedores que ya lo tenían no se borran, solo quedan sin ese dato.');">
                        @csrf
                        @method('DELETE')
