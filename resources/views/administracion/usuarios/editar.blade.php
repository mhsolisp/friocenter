...
<head>
    ...
    <title>Editar usuario — Frío Center</title>
    <style>
...
<main>
    ...
    <div class="card">
        <form action="{{ route('administracion.usuarios.update', $usuario) }}" method="POST">
            ...
            <input type="password" name="password" id="password" minlength="6" placeholder="Dejar en blanco para no cambiarla">

            <label>Rol</label>
            <div class="radio-group">
                <label class="radio-option">
                    <input type="radio" name="rol" value="administracion" onchange="togglePermisos()" {{ $usuario->rol === 'administracion' ? 'checked' : '' }}> Administración
                </label>
                <label class="radio-option">
                    <input type="radio" name="rol" value="taller" onchange="togglePermisos()" {{ $usuario->rol === 'taller' ? 'checked' : '' }}> Taller
                </label>
            </div>

            <div class="permisos-box" id="permisos-box">
                <div class="permiso-item">
                    <input type="checkbox" name="permiso_ver_dia" id="permiso_ver_dia" value="1" {{ $usuario->permiso_ver_dia ? 'checked' : '' }}>
                    <label for="permiso_ver_dia" style="margin:0;">Ver turnos del día</label>
                </div>
                <div class="permiso-item">
                    <input type="checkbox" name="permiso_ver_dias_programados" id="permiso_ver_dias_programados" value="1" {{ $usuario->permiso_ver_dias_programados ? 'checked' : '' }}>
                    <label for="permiso_ver_dias_programados" style="margin:0;">Ver días programados</label>
                </div>
                <div class="permiso-item">
                    <input type="checkbox" name="permiso_ver_historial" id="permiso_ver_historial" value="1" {{ $usuario->permiso_ver_historial ? 'checked' : '' }}>
                    <label for="permiso_ver_historial" style="margin:0;">Ver historial por patente</label>
                </div>
                <p class="nota">Administración siempre tiene acceso a todo, más allá de estos permisos.</p>
            </div>
